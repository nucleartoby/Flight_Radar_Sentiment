...
class OilPricePredictor:
    ...
    @staticmethod
    def _strip_sklearn_only_keys(params: dict, extra_keys=None) -> dict:
        cleaned = dict(params)
        keys_to_remove = ['n_estimators', 'early_stopping_rounds', 'use_label_encoder']
        if extra_keys:
            keys_to_remove.extend(extra_keys)
    ...
    def train_models(self, X: np.ndarray, y: np.ndarray, df: pd.DataFrame, verbose: bool = True) -> Dict[str, Dict]:
        ...
        for name, model in self.models.items():
            ...
            for train_idx, val_idx in tscv.split(X_train_scaled):
                if name == 'xgboost':
                    booster_params = self._strip_sklearn_only_keys(model.get_params())
                    ...
                    cv_pred = cv_model.predict(
                        xgb.DMatrix(X_train_scaled[val_idx]),
                        iteration_range=(0, cv_model.best_iteration + 1))

                else:  # LightGBM
                    lgb_params = self._strip_sklearn_only_keys(model.get_params())
                    ...
                    cv_model = lgb.train(
                        lgb_params, train_data, num_boost_round=1000, valid_sets=[val_data],
            ...
            if verbose:
                self.logger.info(f"{name} - Test RMSE: {results[name]['test_rmse']:.4f}, "
                                  f"R2: {results[name]['test_r2']:.4f}, Iterations: {best_iter}")
